...
用户确认已移开附近物品；22:17 再读盖子状态仍为 `closed`，未见新的开盖事件。
22:13:28 唤醒后到此次检查没有新的 logind 睡眠记录。因此盖子状态异常仍在，
但不能声称当前还在循环睡眠，也不能据此确诊磁性干扰。下一步需受控开合盖，检查状态能否恢复 `open`。
本次仅追加诊断结果；沿用同次诊断已通过的 18 项单元测试、编译与构建结果，不重复无关硬件的软件检查。
