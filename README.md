...
## 主要用途：机器人开发与 Realtime 实时控制

这台电脑同时用于**机器人软件开发**和 **Realtime 机器人实时控制**，实时控制是核心用途之一。项目围绕这两类任务维护 Ubuntu、NVIDIA 驱动和 Razer 硬件环境。

典型工作流程是：开发与构建机器人软件 → 检查实时运行环境 → 根据任务选择普通或 Realtime 内核 → 启动机器人控制并验证控制周期与延迟。开发和实时控制可以在同一台机器上完成，使用哪种内核取决于具体任务。

当前版本提供内核切换与系统管理支持；开发快捷入口及实时调度权限、内存锁定、机器人网络、控制周期和延迟的检查属于后续完善方向，不代表现有版本已完成这些功能或实时性能验证。
...
**用途备注：** 内核切换服务于机器人开发与实时控制：根据开发、调试和控制任务的要求，快速选择普通内核或已安装的 Realtime 内核。该功能负责内核选择与重启切换，机器人控制栈的实时配置和实际控制性能仍需按具体任务验证。
...
**本机系统维护统一入口：** 这台电脑所有系统相关内容统一维护在本仓库，包括系统配置、软件与开发环境安装、驱动、GPU、风扇、电源与睡眠、内核、网络、声音、显示器及机器人 Realtime 环境。相关脚本、配置模板、修复过程、安装与回退步骤和验证记录都在这里归档；凭据及个人数据不上传。
